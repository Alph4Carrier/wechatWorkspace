...
class Orderactive extends Common
{
	public function index()
	{
		$startDay = date("Y-m-d",strtotime("-2 Days"));
		$endDay = date("Y-m-d");
		$orderList = model('common/Order','model')->getOrder($startDay,$endDay,1);
	...
	public function excelExport()
	{
		...
		if(count($ids)){
			$orderList = model('common/Order','model')->getOrderByIds($ids,1);
			$name='活动报名数据表（部分选中数据）_'.$startDay."_至_".$endDay;
		}else{
			$orderList = model('common/Order','model')->getOrderAll($startDay,$endDay,1);
			$name='活动报名数据表_'.$startDay."_至_".$endDay;
		}
		$header = ['姓名',"活动名称","电话","时间"];
		$data = array();
